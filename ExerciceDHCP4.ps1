...
#
#VERSION : 1.1
#COMMENTS : Demande à l'utilisateur les informations pour la création d'une étendue DHCP, avec boite de dialogue et ajout du nb d'étendues a créer.
...
if ($nombreEtendues -match '^\d+$') {
    ...
    for ($i = 1; $i -le $nombreEtendues; $i++) {
        ...
        if ($confirmation -eq "Yes") {
            ...
            Write-Host "L'étendue DHCP '$nomEtendue' a été créée avec succès."
        } else {
            Write-Host "Opération annulée. L'étendue DHCP '$nomEtendue' n'a pas été créée."
        }
    }
} else {
    Write-Host "Nombre d'étendues invalide. Veuillez entrer un nombre entier positif."
}
